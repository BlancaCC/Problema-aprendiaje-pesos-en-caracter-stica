...
csv_file_path  = "src/resultados/Genéticos/AGE/BLX/"
iniciales_nombre = "AGE-BLX-"
process_name = [
...
for i in inicio:final
    ...
    AGE_BLX_LearnerOneNN(data, labels)= AGE_LearnerOneNN(
        data, labels,
     ...
     VerboseCrossValidation(data, labels, 5, AGE_BLX_LearnerOneNN, process_name[i].route)
end